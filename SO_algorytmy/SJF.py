#!/usr/bin/env python
from ProcessClass import processRandomize, ReadData
import Print as P
...
def start():
    ...
    process_queue.sort(key=lambda p_queue: p_queue.burst)
    process_queue.sort(key=lambda p_queue: p_queue.burst+p_queue.arrival)
